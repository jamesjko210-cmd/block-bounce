...
public class BlockBounceGame : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // ── diagnostics + leaderboard submit on game-over ──
        if (lastPhase != s.phase)
        {
            Debug.Log($"[BB] phase {lastPhase} -> {s.phase}");
        ...
        {
            diagTimer = 0f;
            Debug.Log($"[BB] screen={GW}x{GH} camPos={cam.transform.position} camSize={cam.orthographicSize:0.0} | " +
    ...
    void EnsureStyles()
    {
        ...
        stTitle = new GUIStyle(GUI.skin.label) { fontSize = 28, fontStyle = FontStyle.Bold, normal = { textColor = Hex(0x2B1F4D) } };
    ...
    void OnGUI()
    {
        ...
        // Scale the whole HUD up on high-DPI touch devices (mobile only; desktop unchanged).
        ...
        // top-left status
        GUILayout.BeginArea(new Rect(16, 10, Mathf.Min(GW - 32, 640), 44));
        ...
        GUI.Label(new Rect(16, 124, 600, 26), "STAGE GOAL: " + goal, new GUIStyle(stLabel) { fontStyle = FontStyle.Bold });

        // top-right buttons
        if (GUI.Button(new Rect(GW - 180, 14, 80, 28), s.phase == "paused" ? "Resume" : "Pause")) s.TogglePause();
        if (GUI.Button(new Rect(GW - 94, 14, 80, 28), "Quit")) { s.NewGame(true, 50); editingName = false; }
    ...
    void DrawStat(float x, float y, string label, string value, int color)
    {
        ...
        GUI.Label(new Rect(x + 10, y + 24, 116, 34), value, st);
    }


    void DrawLeaderboard()
    {
        float x = GW - 220, y = 90, w = 206;
        GUI.Box(new Rect(x, y, w, 372), GUIContent.none);
        bool live = LeaderboardService.Configured && LeaderboardService.Loaded;
        GUI.Label(new Rect(x + 12, y + 8, w - 20, 20), live ? "LEADERBOARD  ● LIVE" : "LEADERBOARD  (local)", stLabel);
        var lvlStyle = new GUIStyle(stLb) { fontSize = 10, normal = { textColor = Hex(0x6F628F) } };
        ...
        if (live)
        {
            ...
            for (int i = 0; i < top.Count && i < 10; i++)
            {
                ...
                if (me) style.normal.textColor = Hex(0xFF4E78);
    ...
    void ModalBackground()
    {
        ...
        GUI.DrawTexture(new Rect(0, 0, GW, GH), Texture2D.whiteTexture);
        GUI.color = c;
    }
    ...
    void DrawAimHint()
    {
        ...
        GUI.Label(new Rect(GW / 2f - 260, 100, 520, 36), "Move mouse to aim · Click to launch", hintStyle);
